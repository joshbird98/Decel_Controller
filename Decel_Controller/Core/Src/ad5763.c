...
struct ad5763_device set_stage_uV(struct ad5763_device dac, uint8_t channel, float stage_target_uV, uint8_t verbose)
{
	...
	if (channel == 1)
	{
		...
		if (dac_output_target_uV < -4096000) dac_output_target_uV = -4096000;
		dac = set_output_uV(dac, channel, dac_output_target_uV, verbose);
...
void set_output_bits(struct ad5763_device dac, uint8_t channel, uint16_t bits)
{
	...
	if (channel == 1) data_bytes[0] = 0B00010000;
	else if (channel == 2) data_bytes[1] = 0B00010001;
	else return;

	data_bytes[1] = bits >> 8;
	data_bytes[2] = (bits & 0xFF);

	HAL_GPIO_WritePin(&dac.sync_port, dac.sync_pin, GPIO_PIN_RESET);
	HAL_SPI_Transmit(&dac.spi_handle, (uint8_t *)&data_bytes, 3, 100);
	HAL_GPIO_WritePin(&dac.sync_port, dac.sync_pin, GPIO_PIN_SET);
}
